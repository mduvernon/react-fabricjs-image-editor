...
class EventHandler {
	...
	public destroy = () => {
		if (this.handler.editable) {
			...
			this.handler.getObjects().forEach(object => {
				object.off('mousedown', (opt: any) => {
					this.handler.eventHandler.object.mousedown(opt);
				});
	...
	public object = {
		...
		mousedown: (opt: FabricEvent) => {
			const target: any = (opt.selected?.length) ? (opt.selected[0]) : (opt.target);
	...
	public mousemove = (opt: FabricEvent) => {
		...
		if (!this.handler.editable && target) {
			if (target.superType === 'element') {
				return;
			}
			if (target.id !== 'workarea') {
				if (target !== this.handler.target) {
					this.handler.tooltipHandler.show(target);
				}
			} else {
				this.handler.tooltipHandler.hide(target);
			}
		}
		if (this.handler.interactionMode === 'polygon') {
	...
	public mouseout = (opt: FabricEvent) => {
		const target = (opt.selected?.length) ? (opt.selected[0]) : (opt.target);
